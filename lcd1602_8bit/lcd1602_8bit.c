...
/**
 ***************************************************************************************
 *  @breif Конфигурация GPIO для ножек D0-D7
 *  Перед настройкой (GPIOs and AFIOs) нужно включить тактирование порта.
...
void lcd1602_init_GPIO_D0_D7(uint8_t Mode) {
    if (Mode == LCD1602_GPIO_MODE_INPUT) {
        ...
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE0, 0b00 << GPIO_CFGLR_MODE0_Pos);  // 00: Input mode;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF0, 0b10 << GPIO_CFGLR_CNF0_Pos);    // 01: Floating input mode;
        SET_BIT(GPIOA->OUTDR, GPIO_OUTDR_ODR0);                                    // Подтяжка к земле

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE1, 0b00 << GPIO_CFGLR_MODE1_Pos);  // 00: Input mode;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF1, 0b10 << GPIO_CFGLR_CNF1_Pos);    // 01: Floating input mode;

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE2, 0b00 << GPIO_CFGLR_MODE2_Pos);  // 00: Input mode;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF2, 0b10 << GPIO_CFGLR_CNF2_Pos);    // 01: Floating input mode;

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE3, 0b00 << GPIO_CFGLR_MODE3_Pos);  // 00: Input mode;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF3, 0b10 << GPIO_CFGLR_CNF3_Pos);    // 01: Floating input mode;
        ...
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE0, 0b11 << GPIO_CFGLR_MODE0_Pos);  // 11: Output mode, maximum speed: 50MHz;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF0, 0b00 << GPIO_CFGLR_CNF0_Pos);    // 00: General push-pull output mode;

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE1, 0b11 << GPIO_CFGLR_MODE1_Pos);  // 11: Output mode, maximum speed: 50MHz;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF1, 0b00 << GPIO_CFGLR_CNF1_Pos);    // 00: General push-pull output mode;

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE2, 0b11 << GPIO_CFGLR_MODE2_Pos);  // 11: Output mode, maximum speed: 50MHz;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF2, 0b00 << GPIO_CFGLR_CNF2_Pos);    // 00: General push-pull output mode;

        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_MODE3, 0b11 << GPIO_CFGLR_MODE3_Pos);  // 11: Output mode, maximum speed: 50MHz;
        MODIFY_REG(GPIOA->CFGLR, GPIO_CFGLR_CNF3, 0b00 << GPIO_CFGLR_CNF3_Pos);    // 00: General push-pull output mode;
...
void lcd1602_init_display(void) {
    ...
    lcd1602_send_command(0x38);       // FUNCTION SET: 8 бит, 2 строки, шрифт 5x8 точек
...
/**
 ***************************************************************************************
 *  @breif Формирование выхода ножек D0-D7, в соответствии с data
 *  @param  data - Загружаемый байт данных, который выставит биты на ножках D0-D7
 ***************************************************************************************
 */
void lcd1602_set_data(uint8_t data) {
    GPIOA->OUTDR = (GPIOA->OUTDR) & 0xFF00;
    GPIOA->OUTDR = (GPIOA->OUTDR) | data;
}
...
void lcd1602_send_command(uint8_t data) {
    ...
    do {
        ...
        LCD1602_E_CLEAR_BIT;
    } while (READ_BIT(GPIOA->INDR, GPIO_INDR_IDR7) == 1);  // Проверяем BF (PA7)
